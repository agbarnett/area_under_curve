...
for (file in files_to_loop){  # 
  ...
  for (k in 1:nrow(raw_pubmed)){ # loop through abstracts
    ...
    # exclude if the abstract does not have a results section
    results = str_detect(raw_pubmed$abstract[k], 'results_start')
    if(results == FALSE){next}
    
    # abstracts
    aresults = process_abstract(indata = raw_pubmed, k=k, results_section = TRUE) # using the main function 99_main_function_abstract.R
  ...
  # save
  outfile = paste('processed/pubmed.sensitivity.', file_number, '.RData', sep='') # 
  save(abstract.data, 
       aucs, 
