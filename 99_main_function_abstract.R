...
## get sample size
source('99_sample_size.R', echo = TRUE, local=environment())
...
if (any_auc == TRUE){
  ...
  for_auc_clean = str_remove_all(for_auc, statistics_patterns) # is removing useful stuff  
  ...
  source('99_auc_next_numbers.R', echo = TRUE, local=environment())

  ## AUC as a percent - using nearby words
  source('99_auc_percent.R', echo = TRUE, local=environment())

  ## "respectively" - uses for_auc
  source('99_auc_respectively.R', echo = TRUE, local=environment())
  
  ## AUC as a confidence interval
  source('99_auc_confidence_intervals.R', echo = TRUE, local=environment())
  
  ## AUC as a range/pair, using `?. ?` for any hyphen
  source('99_auc_pair.R', echo = TRUE, local=environment())

  # likely to move above    
  ## For sentences with combined statistics, split on statistics key words
  source('99_auc_nearby_statistics.R', echo = TRUE, local=environment())
  ## if I add any more screens below, then I need to update for_auc without sub-sentences ##
  
  ## combine all sources of AUCs
  aucs = bind_rows(auc_numbers, auc_numbers2, aucs1_range_split, aucs1, aucs2, aucs3, aucs_percent, aucs_sub, auc_respectively) # combine all searches
  if(nrow(aucs) > 0){
    #
